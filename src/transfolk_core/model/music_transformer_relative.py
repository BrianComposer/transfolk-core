...
class RelativePositionBias(nn.Module):
    """
    Sesgo posicional relativo tipo T5 simplificado.
    """
...
class CausalSelfAttentionRelative(nn.Module):
    def __init__(self, d_model: int, n_heads: int, dropout: float, max_seq_len: int, max_distance: int = 128):
        super().__init__()
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model: int, n_heads: int, d_ff: int, dropout: float, max_seq_len: int, max_distance: int = 128):
        super().__init__()
        self.ln1 = nn.LayerNorm(d_model)
        self.attn = CausalSelfAttentionRelative(d_model, n_heads, dropout, max_seq_len, max_distance=max_distance)
...
class MusicTransformerRelative(BaseMusicModel):
    """
    Transformer decoder-only con atención relativa y padding mask transversal.
    """
